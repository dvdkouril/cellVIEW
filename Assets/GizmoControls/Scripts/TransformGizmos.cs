...
public class TransformGizmos : MonoBehaviour {
	...
	public float    Spread=1f;  //Spread of lines
	...
	void Update(){
		if (ScaleWithCamera) {
			...
			Spread = Mathf.Clamp (DistanceToCam, 1, 3);
		}
	}
	void SetMaterial(){
	...
	void OnRenderObject(){
		...
		if(SelectedType==MOVETYPE.NONE&& hit.x>0f && hit.x<=0.3f*Spread && hit.z>0 && hit.z<=0.3f*Spread){
			panY=SelectedColor;
		...
		if(SelectedType==MOVETYPE.NONE && hit.x>0f && hit.x<=Spread && Mathf.Abs(hit.z)<SelectedSencsitivity ){ //case x
		   selX=SelectedColor;
		...
		if(SelectedType==MOVETYPE.NONE && hit.z>0f && hit.z<=Spread && Mathf.Abs(hit.x)<SelectedSencsitivity ){//case  z
		    selZ=SelectedColor;
		...
		if(ScaleOption){
			if(SelectedType==MOVETYPE.NONE&& hit.x>0f && hit.x<=0.3f*Spread && hit.z>0 && hit.z<=0.3f*Spread && hit.x+hit.z-0.3f*Spread<=0f){
		...
		if(SelectedType==MOVETYPE.NONE&& rlens>(1-Sencsitivity) && rlens<(1+Sencsitivity) ){
				circleX=Color.red;
				circleY=Color.green;
				circleZ=Color.blue;
	    ...
	    if(TransformationOption){
		if(SelectedType==MOVETYPE.NONE&&hit.x>0f && hit.x<=0.3f*Spread && hit.y>0 && hit.y<=0.3f*Spread){
			panZ=SelectedColor;
		...
		if(SelectedType==MOVETYPE.NONE && hit.x>0f && hit.x<=Spread && Mathf.Abs(hit.y)<SelectedSencsitivity ){ //case x
		   selX=SelectedColor;
		...
		if(SelectedType==MOVETYPE.NONE && hit.y>0f && hit.y<=Spread && Mathf.Abs(hit.x)<SelectedSencsitivity){ //case y
		   selY=SelectedColor;
		...
		if(ScaleOption){
			if(SelectedType==MOVETYPE.NONE&&hit.x>0f && hit.x<=0.3f*Spread && hit.y>0 && hit.y<=0.3f*Spread&& hit.x+hit.y-0.3f*Spread<=0f){
			panZ=SelectedColor;
		...
		if(TransformationOption){
		if(SelectedType==MOVETYPE.NONE&& hit.z>0 && hit.z<=0.3f*Spread && hit.y>0 && hit.y<=0.3f*Spread ){
			panX=SelectedColor;
		...
		if(SelectedType==MOVETYPE.NONE && hit.z>0f && hit.z<=Spread && Mathf.Abs(hit.y)<SelectedSencsitivity*Spread){//case  z
		   selZ=SelectedColor;
		...
		if(SelectedType==MOVETYPE.NONE && hit.y>0f && hit.y<=Spread && Mathf.Abs(hit.z)<SelectedSencsitivity*Spread ){ //case y
		   selY=SelectedColor;
		...
		if(ScaleOption){
		if(SelectedType==MOVETYPE.NONE&& hit.z>0 && hit.z<=0.3f*Spread && hit.y>0 && hit.y<=0.3f*Spread && hit.y+hit.z-0.3f*Spread<=0f){
			panX=SelectedColor;
    ...
    Vector3 prjleft =Vector3.Cross(Vector3.right,camdir);
	prjleft.Normalize();
	prjleft*=Spread;
	Vector3 prjfwrd=Vector3.Cross(prjleft,Vector3.right);
	prjfwrd.Normalize();	
	prjfwrd*=Spread;
    ...
    prjleft =Vector3.Cross(Vector3.up,camdir);
	prjleft.Normalize();
	prjleft*=Spread;
	prjfwrd=Vector3.Cross(prjleft,Vector3.up);
	prjfwrd.Normalize();
	prjfwrd*=Spread;
	DrawCircle(circleY,prjleft,prjfwrd);
	uForward=prjfwrd;	

    prjleft =Vector3.Cross(Vector3.forward,camdir);
	prjleft.Normalize();
	prjleft*=Spread;
	prjfwrd=Vector3.Cross(prjleft,Vector3.forward);
	prjfwrd.Normalize();
	prjfwrd*=Spread;
	...
	if(TransformationOption){	
	DrawAxis(Vector3.right*Spread,Vector3.up,Vector3.forward,0.04f*Spread,0.9f,selX);	
	DrawAxis(Vector3.up*Spread,Vector3.right,Vector3.forward,0.04f*Spread,0.9f,selY);
	DrawAxis(Vector3.forward*Spread,Vector3.right,Vector3.up,0.04f*Spread,0.9f,selZ);
		
	
	DrawQuad(0.3f*Spread,false,Vector3.forward,Vector3.up,panX);
	DrawQuad(0.3f*Spread,false,Vector3.right,Vector3.forward,panY);
	DrawQuad(0.3f*Spread,false,Vector3.right,Vector3.up,panZ);
	...
	void DrawCam(Color col, float sang,float eng ,MOVETYPE dtype){
		...
		switch(dtype){
		   ...
		   GL.Vertex3(0,0,0);
	       GL.Vertex3( 0,Mathf.Sin(ang)*Spread,Mathf.Cos(ang)*Spread);
		   ang=sang+(eng)*(i+1)/20;
		   GL.Vertex3( 0,Mathf.Sin(ang)*Spread,Mathf.Cos(ang)*Spread);
		   ...
		   GL.Vertex3(0,0,0);
	       GL.Vertex3( Mathf.Cos(ang)*Spread,0,Mathf.Sin(ang)*Spread);
		   ang=sang+(eng)*(i+1)/20;
		   GL.Vertex3( Mathf.Cos(ang)*Spread,0,Mathf.Sin(ang)*Spread);
		   ...
		   GL.Vertex3(0,0,0);
	       GL.Vertex3( Mathf.Sin(ang)*Spread,Mathf.Cos(ang)*Spread,0);
		   ang=sang+(eng)*(i+1)/20;
		   GL.Vertex3( Mathf.Sin(ang)*Spread,Mathf.Cos(ang)*Spread,0);
